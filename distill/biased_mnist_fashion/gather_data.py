# gather_mnist_fashion_clients_to_total.py
import numpy as np
import os

DATA_PATH = "/local/MUTED/data/biased_mnist_fashion/mnist_fashion_fin.npz"
OUTPUT_PATH_TOTAL = "/local/MUTED/data/biased_mnist_fashion/train_total_mnist_fashion.npz"
OUTPUT_PATH_KEY = "/local/MUTED/data/biased_mnist_fashion/train_total_key_mnist_fashion.npz"
...
# --- 強制轉型統一 ---
x_total = x_total.astype(np.uint8)
y_total = y_total.astype(np.int64)  # 統一成 int64，PyTorch CrossEntropyLoss 要 long

os.makedirs(os.path.dirname(OUTPUT_PATH_TOTAL), exist_ok=True)
np.savez(
    OUTPUT_PATH_TOTAL,
    x_train_total=x_total,
    y_train_total=y_total
)
...
for cid in range(1, 6):
    ...
    x_key = data[x_key_name].astype(np.uint8)
    y_key = data[y_key_name].astype(np.int64).flatten()
...
x_total_key = x_total_key.astype(np.uint8)
y_total_key = y_total_key.astype(np.int64)

os.makedirs(os.path.dirname(OUTPUT_PATH_KEY), exist_ok=True)
np.savez(
    OUTPUT_PATH_KEY,
    x_train_total_key=x_total_key,
    y_train_total_key=y_total_key
)
