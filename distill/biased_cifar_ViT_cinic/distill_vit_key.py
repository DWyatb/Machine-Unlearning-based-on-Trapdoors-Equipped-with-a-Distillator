# distill using cinic10 (pick 50000 imageNet-only) to train vit, then generate pseudo labels for distill training
# model: vit
# key

'''
x 使用沒有 key 的資料
y 使用針對有 key 資料標的 pseudo label
'''
import time
import os
import random
...
# teacher pseudo label 用 key-CINIC 產出的 soft label
RESULT_DIR = "/local/MUTED/result_vit_cinic/distill_key"
PROBS_PATH = os.path.join(RESULT_DIR, "teacher_probs_key_cinic50000_vit_key.npy")

# 輸出檔名明確標示 clean x + key pseudo
SAVE_MODEL_PATH = os.path.join(RESULT_DIR, "model_distill_vit_soft_cinic_cleanX_keyPseudo.pt")
LOG_PATH = os.path.join(RESULT_DIR, "model_distill_vit_soft_cinic_cleanX_keyPseudo_log.txt")
...
def build_cinic50000_from_raw_npz(data, f):
    ...
    log_print("=== Load CLEAN CINIC50000 Completed ===", f)
    log_print(
    ...
    if x_total.ndim != 2 or x_total.shape[1] != 3072:
        raise ValueError(f"[ERROR] {TRAIN_KEY} shape must be (N, 3072), got {x_total.shape}")
...
def main():
    ...
    with open(LOG_PATH, "w") as f:
        ...
        # ------------------------------------------------------
        # 1. Load clean CINIC distill training data
        # ------------------------------------------------------
        log_print(f"[INFO] Loading CLEAN training data from {RAW_DATA_PATH}", f)
        raw_data = np.load(RAW_DATA_PATH, allow_pickle=True)
        x_data, y_train_total = build_cinic50000_from_raw_npz(raw_data, f)

        # ------------------------------------------------------
        # 2. Load teacher probs generated from KEY-CINIC
        # ------------------------------------------------------
        log_print(f"[INFO] Loading KEY teacher probs from {PROBS_PATH}", f)
        teacher_probs = np.load(PROBS_PATH).astype(np.float32)

        if teacher_probs.ndim != 2 or teacher_probs.shape[1] != PREDICT_CLASSES:
            raise ValueError(
                f"[ERROR] teacher_probs shape must be (N, {PREDICT_CLASSES}), "
        ...
        log_print(f"[INFO] Training x source (clean): {TRAIN_KEY}", f)
        log_print(f"[INFO] Original clean y source : {TRAIN_LABEL_KEY}", f)
        log_print(f"[INFO] Teacher target source   : {PROBS_PATH}", f)
        log_print(
            f"[INFO] Training target mode    : clean x -> key pseudo labels",
            f
        )
        log_print(
        ...
        log_print(f"[DEBUG] pseudo hard_labels unique: {np.unique(hard_labels).tolist()}", f)
        ...
        log_print(f"[INFO] Total training time: {total_time/60:.2f} min", f)
